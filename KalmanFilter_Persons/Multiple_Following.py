...
LES COMMENTAIRES SONT SIMILAIRES AU CODE SUR LES ABEILLES
"""
...
dataset="PETS09-S2L1"                        #Camera fixe :PETS09-S2L1  #En mouvement : ETH-BAHNHOF
...
data=genfromtxt(fichier_label, delimiter=',')   #récupération des données à partir du fichier
id_frame=0   
...
for image in glob.glob(dir_images+"*.jpg"):
    ...
    for id_obj in range(len(objets_points)):
        ...
        predict_etat=objets_KF[id_obj].predict()
        predict_etat=np.array(predict_etat, dtype=np.int32)
        etat = []
        
        for i in range(len(predict_etat)):
            etat.append(predict_etat[i][0])
        if len(objets_points)!= len(objets_historique):  
            print("DIFF",len(objets_points)," ",len(objets_historique))
        ...
        #Si l'option rectangle est activéee
        if rectangle:
            cv2.rectangle(frame,
                          (int(etat[0]-etat[4]/2), int(etat[1]-etat[5]/2)),
                          (int(etat[0]+etat[4]/2), int(etat[1]+etat[5]/2)),
    ...
    for d in data[mask, :]:           
        ...
        if rectangle:
            cv2.rectangle(frame, (int(d[2]), int(d[3])),
                          (int(d[2]+d[4]), int(d[3]+d[5])),
                          (0, 255, 0),
                          2)
        xm=int(d[2]+d[4]/2)
        ym=int(d[3]+d[5]/2)
...
# calcul des distances
    ...
    for point_id in range(len(points)):
        if nouveaux_objets[point_id]:
            ...
            objets_KF.append(KalmanFilter(0.5,
